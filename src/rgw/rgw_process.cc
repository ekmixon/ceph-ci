...
int process_request(rgw::sal::Store* const store,
		    ...
		    rgw::dmclock::Scheduler *scheduler,
                    ...
                    rgw::sal::LuaScriptManager* lua_script_manager,
                    int* http_ret)
{
  ...
  }
  {
    ...
    std::string script;
    auto rc = rgw::lua::read_script(s, lua_script_manager, s->bucket_tenant, s->yield, rgw::lua::context::preRequest, script);
    if (rc == -ENOENT) {
  ...
  if (op && s->trace) {
    ...
    std::string script;
    auto rc = rgw::lua::read_script(s, lua_script_manager, s->bucket_tenant, s->yield, rgw::lua::context::postRequest, script);
    if (rc == -ENOENT) {
      // no script, nothing to do
    } else if (rc < 0) {
