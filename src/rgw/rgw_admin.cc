...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::PUBSUB_EVENT_RM) {
  ...
  if (opt_cmd == OPT::SCRIPT_PUT) {
    ...
    auto manager = store->get_lua_script_manager();
    rc = rgw::lua::write_script(dpp(), manager.get(), tenant, null_yield, script_ctx, script);
    if (rc < 0) {
  ...
  if (opt_cmd == OPT::SCRIPT_GET) {
    ...
    auto manager = store->get_lua_script_manager();
    std::string script;
    const auto rc = rgw::lua::read_script(dpp(), manager.get(), tenant, null_yield, script_ctx, script);
    if (rc == -ENOENT) {
  ...
  if (opt_cmd == OPT::SCRIPT_RM) {
    ...
    auto manager = store->get_lua_script_manager();
    const auto rc = rgw::lua::delete_script(dpp(), manager.get(), tenant, null_yield, script_ctx);
    if (rc < 0) {
