...
int read_script(const DoutPrefixProvider *dpp, rgw::sal::LuaScriptManager* manager, const std::string& tenant, optional_yield y, context ctx, std::string& script)
{
  return manager->get(dpp, y, script_oid(ctx, tenant), script);
}

int write_script(const DoutPrefixProvider *dpp, rgw::sal::LuaScriptManager* manager, const std::string& tenant, optional_yield y, context ctx, const std::string& script)
{
  return manager->put(dpp, y, script_oid(ctx, tenant), script);
}

int delete_script(const DoutPrefixProvider *dpp, rgw::sal::LuaScriptManager* manager, const std::string& tenant, optional_yield y, context ctx)
{
  return manager->del(dpp, y, script_oid(ctx, tenant));
}
